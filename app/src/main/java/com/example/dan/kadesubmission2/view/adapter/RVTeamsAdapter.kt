...
class RVTeamsAdapter(private val mContext: Context) : RecyclerView.Adapter<RVTeamsAdapter.RVHolderTeams>(){
    val inflater : LayoutInflater

    var listTeams : List<TeamLogo> = mutableListOf()
    init {
        inflater = LayoutInflater.from(mContext)
    }

    fun setupListTeams(teams : List<TeamLogo>){
        this.listTeams = teams
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RVTeamsAdapter.RVHolderTeams {
    ...
    override fun onBindViewHolder(holder: RVTeamsAdapter.RVHolderTeams, position: Int) {
...
//        val cardViewListMatch = itemView.findViewById<CardView>(R.id.cv_recycler_item)
        ...
        fun bind(teams : TeamLogo){
            Picasso.get().load(teams.linkClubLogo).into(ivClubLogo)
            tvClubName.text = teams.teamName

            itemView.setOnClickListener{
                mContext.startActivity(Intent(mContext, TeamDetailsActivity::class.java)
                        .putExtra("ID_CLUB",teams.idTeam)
                        .putExtra("CLUB_NAME",teams.teamName)
                        .putExtra("CLUB_LOGO_URL",teams.linkClubLogo)
                        .putExtra("CLUB_STADIUM",teams.stadiumName)
                        .putExtra("CLUB_YEARS",teams.intFormedYear)
                )
            }
        }
